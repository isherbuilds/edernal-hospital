...
const PATIENT_SEX_OPTIONS = ["male", "female", "other", "unknown"] satisfies PatientOutput["sex"][];
...
function RouteComponent() {
  ...
  async function handleRegister(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (
        ...
        const matches = await patientSearch.mutateAsync({
  ...
  async function moveToken(tokenId: string, status: QueueTokenOutput["status"]) {
    if (!canManageQueue) {
        ...
        <FieldGroup>
              ...
              <div className="flex flex-wrap gap-2">
                {organizationOptions.map((organization) => (
                  <Button
                    ...
                    onClick={() => setTenantId(organization.id)}
                  >
          ...
          <Field>
            ...
            <Input
              ...
              className="max-w-sm"
              value={tenantId}
              onChange={(event) => setTenantId(event.currentTarget.value.trim())}
              placeholder="org_..."
            />
          </Field>
        </FieldGroup>
        ...
        <div className="rounded-lg border p-4">
          ...
          {canRegister ? (
            <form className="flex flex-col gap-4" onSubmit={handleRegister}>
              ...
              <Button
                type="submit"
                disabled={!tenantReady || quickRegister.isPending || patientSearch.isPending}
              >
