...
**Reading guide:** every phase states *why it exists*, then a step-by-step checklist, then an exit gate. A phase is done when its gate passes — not when its checklist looks done. If the date compresses, cut from the *end* of Phase 3 and Phase 4's P1 items, never from Phase 5 (trust) or the gates.

The repo today is a pristine tsu-stack starter (TanStack Start + Hono + oRPC + Drizzle + Better Auth, Coolify compose files). The 125-table schema pack in `plans/` is a **reference menu** — pull tables per phase, simplified ([ADR-0002](./adr/0002-relational-first-fhir-at-the-seam.md)).
...
- [ ] Tenant context middleware: resolve tenant from session on every request; set Postgres session GUCs (`app.tenant_id`, `app.user_id`) per transaction — adapt `002_rls_context.sql` from the schema pack.
- [ ] RLS: enable + FORCE on all tenant-scoped tables from the first migration; adapt the schema pack's policy loop.
- [ ] `audit_events` table (append-only; app DB role has INSERT/SELECT only) + `audit()` helper wired into the procedure factory so PHI procedures emit events without per-endpoint boilerplate.
...
**Why:** the pilot's output is *learning*, and learning needs instrumented, supported, real usage — not a launch event.
