# 0007: HMS owns operational billing; general ledger lives in edernal-books via a one-way export seam; inventory has exactly one owner
...
2. **Integration is a one-way, batch-shaped export seam — never a runtime dependency.** At pilot: accountant-grade exports from HMS (GST-ready invoice/collections registers, day-close summary as CSV) matching what the incumbent gives the CA. Later: the same day-close feed posts journal entries into edernal-books through its Phase-6 public API. Invoice printing and payment capture at the hospital counter must never block on the accounting system being up.
...
- HMS Phase 3 (money loop) gains the accountant-export checklist items; the strategic Books integration is a vision-phase item gated on books Phase 6, not on HMS code.
