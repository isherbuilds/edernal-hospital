...
## Deferred RLS pattern (reference only)

The session-GUC + policy-loop pattern from the original schema pack. [ADR-0003](../adr/0003-single-vps-app-tenancy.md) now chooses app-enforced tenant scoping for the pilot, so this is **not** a Phase-0 checklist item. Keep it as a future hardening reference if hospital #2, self-serve SaaS, or a customer contract re-opens database-enforced isolation.
 ...
 * - Pilot clinical data lives in relational domain tables. FHIR tables are a deferred export seam, not the canonical store.
...
Patients, encounters, notes, prescriptions, orders. Feeds Phases 1–3 — simplify heavily; the pack's shapes assume the full PRD suite and still reference deferred FHIR projections in places. For pilot implementation, prefer plain relational `patients` / `encounters` tables and do not import the `fhir.ts` module.
